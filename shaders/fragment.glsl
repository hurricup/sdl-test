...
void main(){
    ...
    vec3 diffuse_color = light_color * (diffuse * material.diffuse) * light_distance_decay;

    // specular color
    vec3 reflect_direction = reflect(-light_direction, norm);
    float specular = pow(max(dot(camera_direction, reflect_direction), 0.0), material.shininess);
    vec3 specular_color = light_color * (specular * material.specular) * light_distance_decay;
